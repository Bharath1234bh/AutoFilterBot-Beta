...
# Channels
INDEX_CHANNELS = [int(index_channels) if index_channels.startswith("-") else index_channels for index_channels in environ.get('INDEX_CHANNELS', '1002071195970').split()]
...
OPENAI_API = environ.get('OPENAI_API', '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')
if len(OPENAI_API) == 0:
...
BIN_CHANNEL = environ.get("BIN_CHANNEL", "-1002071195970")
if len(BIN_CHANNEL) == 0:
    logging.error('BIN_CHANNEL is missing, exiting now')
    exit()
else:
    BIN_CHANNEL = int(BIN_CHANNEL)
URL = environ.get("URL", "tamilan-bot-kitj.onrender.com")
if len(URL) == 0:
